...
public interface IEnergyTarget extends IChanneledTarget<IEnergyNetwork, Long> {
    ...
    public static IEnergyTarget ofTile(PartTarget partTarget, IAspectProperties properties, long amount) {
        ...
        return new EnergyTargetCapabilityProvider(Block.class, tile == null ? ICapabilityGetter.forBlock(target.getPos().getLevel(true), target.getPos().getBlockPos(), null, null) : ICapabilityGetter.forBlockEntity(tile), target.getSide(), network, properties, amount, partState);
    }
